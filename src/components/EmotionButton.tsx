...
const EmotionButton: React.FC<EmotionButtonProps> = ({ label, onClick, color, selected, disabled, size, className, defaultStyle = true }) => {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={classNames(`h-fit text-white bg-gradient-to-br to-slate-100 border hover:to-white hover:text-white focus:outline-none font-bold text-center mr-2 transition-all ease-in duration-100`,
        emotionClass,
